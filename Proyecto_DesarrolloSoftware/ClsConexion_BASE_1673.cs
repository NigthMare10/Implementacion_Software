...
namespace Proyecto_DesarrolloSoftware
{
    namespace Proyecto_DesarrolloSoftware
    {
        internal class ClsConexion
        {
            ...
            public DataTable FillDataGrid(string query)
            {
            ...
            public DataTable ObtenerVentasMeses(int year, int mes1, int mes2, int mes3)
            {
                ...
                SELECT 
                CASE 
                    WHEN MONTH(Factura_fecha) = @Mes1 THEN MONTHNAME(Factura_fecha)
                    WHEN MONTH(Factura_fecha) = @Mes2 THEN MONTHNAME(Factura_fecha)
                    WHEN MONTH(Factura_fecha) = @Mes3 THEN MONTHNAME(Factura_fecha)
                END AS NombreMes,
                SUM(Factura_total) AS TotalVentas
            FROM Factura
            WHERE YEAR(Factura_fecha) = @Year AND MONTH(Factura_fecha) IN (@Mes1, @Mes2, @Mes3)
            OR (YEAR(Factura_fecha) = @Year - 1 AND MONTH(Factura_fecha) IN (11, 12) AND @Mes1 IN (1, 2) AND (MONTH(Factura_fecha) = @Mes2 OR MONTH(Factura_fecha) = @Mes3))
            GROUP BY NombreMes;";
                ...
                try
                {
                    using (MySqlConnection con = new MySqlConnection(connectionString))
                    {
                        using (MySqlCommand cmd = new MySqlCommand(query, con))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@Mes3", mes3);
